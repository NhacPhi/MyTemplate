        ...
        if "CharacterStat" in all_sheets:
            ...
            for _, row in df.iterrows():
                ...
                if char_id in character_data:
                    ...
                    calc_cr = 60 if cls == 'Boss' else (base_st['crit_rate'] + (r_tier * inc_st['crit_rate']) + b_cr)
                    calc_cd = 50 if cls == 'Boss' else (base_st['crit_dmg'] + (r_tier * inc_st['crit_dmg']) + b_cd)
                    calc_pen = 20 if cls == 'Boss' else (base_st['penetration'] + (r_tier * inc_st['penetration']) + b_pen)
                    calc_res = 25 if cls == 'Boss' else (base_st['crit_dmg_res'] + (r_tier * inc_st['crit_dmg_res']) + c_res_bonus + b_res)
