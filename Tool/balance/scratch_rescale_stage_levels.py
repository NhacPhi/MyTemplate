...
for attempt in range(5):
    try:
        ...
        for row in ws.iter_rows(min_row=2):
            b_id = str(row[0].value).strip()
            slot = int(row[1].value)
            key = (b_id, slot)
            if key in level_updates:
                old_lvl = row[3].value
                new_lvl = level_updates[key]
                row[3].value = new_lvl
                print(f"Updated {b_id} slot {slot} level: {old_lvl} -> {new_lvl}")
        wb.save('Tool/data/GameConfig.xlsx')
        print("Successfully saved updated StageEnemies to GameConfig.xlsx!")
        break
