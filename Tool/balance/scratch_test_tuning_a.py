...
def test_config(atk_w, hp_w, def_w, crit_s, pen_s, shred_s, spd_b, spd_s, ur_m, ssr_m):
    for cid in ['SunWukong', 'ErlangShen', 'ThirdPrinceNezha', 'BullDemonKing']:
        ...
        crit_power = total_atk * (total_cr / 100.0) * (total_cd / 100.0) * crit_s
        pen_power = total_atk * (total_pen / 100.0) * pen_s + (total_def_shred * shred_s)
        offensive_rating = (total_atk * atk_w) + crit_power + pen_power
        
        crit_res_power = total_hp * (total_crit_res / 100.0) * 0.3
        defensive_rating = (total_hp * hp_w) + (total_def * def_w) + crit_res_power
        
        speed_factor = spd_b + (total_spd / 100.0) * spd_s
        r_mult = {'UR': ur_m, 'SSR': ssr_m, 'SR': 0.85, 'R': 0.70}.get(rare, 1.0)
        ...
        print(f"  {cid:<20} ({rare:<3} {c_cfg['type']:<9}): CP = {cp:>7,}")

print("\n--- Tuning Option A (Standard RPG Balance: ATK 5.0, HP 1.2, DEF 8.0) ---")
test_config(atk_w=5.0, hp_w=1.2, def_w=8.0, crit_s=1.2, pen_s=0.8, shred_s=30, spd_b=0.6, spd_s=0.4, ur_m=1.25, ssr_m=1.0)
