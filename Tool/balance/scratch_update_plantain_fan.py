...
desc_vi = 'Tăng {0}% Sát Thương Bạo Kích và {1}% Tấn Công. Đòn tấn công kỹ năng giảm {2}% Phòng Thủ của mục tiêu trong 2 hiệp và gây thêm {3}% Sát Thương lên kẻ địch đang chịu hiệu ứng bất lợi.'
desc_en = 'Increases CRIT DMG by {0}% and ATK by {1}%. Skill attacks reduce target DEF by {2}% for 2 turns and deal {3}% additional Damage to debuffed enemies.'
...
for attempt in range(5):
    try:
        ...
        wb_gc.save(gc_path)
        print("Successfully saved GameConfig.xlsx for Plantain_Fan!")
        break
...
for attempt in range(5):
    try:
        ...
        print("Successfully saved Localizations.xlsx for Plantain_Fan!")
        break
    except Exception as e:
